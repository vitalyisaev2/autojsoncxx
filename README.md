...
* [libclang] (http://clang.llvm.org) and its python binding 
...
For Windows users, may god help you. Or you could try release 0.2.0 where the `libclang` is not needed.
...
* Clang 3.5 on Mac OS X (11.9)
* GCC 4.9 (Homebrew) on Mac OS X (11.9)
* GCC 4.8 on Ubuntu 14.04.1 (x86/x64)
* MSVC 10 (x86) on Windows 7 (no code generation is tested on Windows)
* MSVC 11/12 (x86/x64) on Windows 7 (no code generation is tested on Windows)
...
* Object types: any class that contains only non-ignored fields of supported type and scanned by the script `autojsoncxx.py`.
